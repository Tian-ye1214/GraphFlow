...
| 扇出 / 并发 / 重试 | `fanout=` / `conc=` / `retries=` | fanout_n / concurrency / retries | `conc=4` | **fanout 仅 llm**；conc/retries 适用 llm/qc/http |
| 采样参数 | `temp=` / `top_p=` / `max_tokens=` / `timeout=` / `json_mode=` | params.* | `temp=0` `json_mode=true` | llm / qc |
| 思考 | `think=on\|off` / `effort=low..max` | params.thinking_enabled / params.reasoning_effort | `think=on effort=high` | llm / qc |
...
| 输出存为数据集 | `save_as=名`（空串=关闭，**同时清空数据集名**） | save_as_dataset + dataset_name | `save_as=结果集` | output |
...
⚠️ `outs=`（output_columns）只是**声明** JSON 模式下产出哪些列（供下游 `{{列}}`/血缘识别）；运行时引擎对 mode=json 节点把模型返回 JSON 的**所有键原样铺开**（不按 outs 过滤也不补齐）——`outs` 要和提示词让模型真正返回的 JSON 键**自己对齐**。

⚠️ 思考键只对 **llm/qc** 有意义（http 不调 LLM）；**没有 agent 节点类型**，RedLotus Agent 的思考是按角色写死(xhigh)配在会话上，不经 `node set`。

## `gf node show <ID>`

打印节点完整 JSON（含 type/position/config），核对刚 set 的字段是否落对。

## `gf node try <ID>`（试跑，零副作用）

用节点真实输入的小样本跑该节点逻辑，看渲染后的提示词与模型实际产出/质检判定——定稿提示词前先试跑据实打磨。**不落库、不计入正式运行**。支持 llm/qc/http/auto 节点。

```powershell
gf node try llm_synth_1            # 调真实模型，看 ≤3 行的渲染提示词+产出列
gf node try llm_synth_1 --render   # 只渲染提示词不调模型（免费，查 {{列}} 是否对、有无缺失列）
gf node try qc_1                   # 看判定结果与各模型理由
gf node try auto_process_1 --limit 10   # 本地变换(dedup/filter/agent代码)，看转换后产出行
gf node try http_1                 # GET/HEAD 直接试；非读方法需 --allow-side-effects
gf node try llm_synth_1 --json     # 输出原始 JSON
```

⚠️ 试跑用「已保存」的节点配置；改了配置先 `node set`/`node prompt` 落库再 try。输入样本优先取最近一次运行的上游产出，没有运行则回退输入数据集。

## `gf node prompt <ID> <--system|--user> <--file FILE | --edit | - | --library …>`（长提示词）
...
- 必须**四选一**指定来源：`--file FILE` / `--edit` / `-`（stdin）/ `--library`（从提示词库取）——四者互斥且必选其一。
- 从**提示词库**取：`--library <提示词id或名>` 配 `--ref`（引用，运行时取最新版）或 `--copy`（复制当前正文进来，默认）。引用写 `system_prompt_ref`/`user_prompt_ref`，运行时解析；缺失则整 run 报错。**`--ref`/`--copy` 仅配合 `--library` 生效**，不带 `--library` 时被忽略（且写内联文本恒清除该槽位引用）。库的维护见 **gf-prompt**。

## `gf op`（自动处理节点的操作列表，位置参数不是 key=value）

```
gf op add <节点> dedup [列1,列2]      # 缺省全列去重
gf op add <节点> filter <列> <min_len|max_len|contains|not_contains|regex|not_empty|equals> <值>
...
⚠️ `op add auto_process_1 dedup col=q` 是错的——写 `dedup q`。filter 的 `min_len`/`max_len` 值会自动转 int；`not_empty` 忽略值但仍需占位一个值凑满三参（如 `filter q not_empty x`），`equals` 用 `s==str(值)` 比较。
