...
| 输出产量上限 | `count=N`（空串=不限） | count | `count=100` | output |
...
⚠️ 数字键自动转型：`fanout`/`conc`/`retries`/`max_tokens`/`timeout`/`pass_k`/`max_rounds`/`count` 转 int，`temp`/`top_p` 转 float，`json_mode` 取真值（true/1/yes），`think` 取真值（on/true/1/yes）。

⚠️ `count`（output 产量上限，≥1 或留空）：设了之后**上游合成产够 count 条就停掉大模型**（省调用），最终输出截到 ≤count；QC 淘汰/失败的**不补**（多少要多少）。扇出>1 时按整输入行向上取整、合成可能略多产但输出仍截到 count。仅对「上游有 llm 合成」的链省成本。
