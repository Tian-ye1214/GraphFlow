...
| `gf watch [运行ID]` | 跟随进度（原地刷新，阻塞到终态）；缺省取当前工作流最近一次运行 |
| `gf run-show <运行ID>` | **非阻塞一次性**打印某次运行的状态/统计/逐节点汇总(done·total·失败数)，不跟随 |
| `gf cancel <运行ID>` | 仅 queued/running 可取消，否则报「当前状态 X 不可取消」并退出码 1 |
| `gf rerun <运行ID>` | 重跑失败行（失败行重新排队，下游节点数据自动重算）。**仅对已结束（完成/失败/已取消）且确有失败行的运行有效**，否则报「运行尚未结束」或「没有失败行」退出码 1 |
...
| 命令 | 说明 |
|---|---|
| `gf rows <运行ID> [--node 节点ID] [--failed] [--page N]` | 看某节点结果行（每页 20）。`--node` 缺省取**第一个输出节点**；`--failed` 看失败行——只给行号 `row_idx`、错误 `error`、重试 `attempt`，**不含行数据**（看失败样本内容用 `gf qc` 或导出后人工核） |
| `gf logs <运行ID> [--model]` | 看运行日志时间线；`--model` 改看该运行的模型对话（source/节点/模型名 + **完整请求/回复正文 + tokens**） |
| `gf model-logs [--source S] [--run ID] [--node ID] [--limit N]` | **跨运行**看模型调用日志（含请求/回复正文 + tokens）；按来源/运行/节点过滤，默认最近 50 条 |
| `gf qc <运行ID> [--download] [-o 文件]` | 看质检指标（各 qc 节点首轮通过率）+ 失败样本；`--download` 把最终失败样本全量落 jsonl（缺省 `run<ID>_qc_failures.jsonl`） |

`gf qc` 不带 `--download` 打印每个 qc 节点「首轮通过 m/n（x%）」与失败样本（样本 + 各判定模型的 `status:reason`）。⚠️ 非下载视图的失败样本**最多前 200 条**（服务端 limit），且**汇总该运行所有 qc 节点**（CLI 无 `--node` 开关）；要全量请用 `--download`（jsonl，按节点过滤需走 REST `/api/runs/<id>/qc-failures?node_id=...`）。
