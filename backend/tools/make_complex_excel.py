"""生成一个「足够刁钻」的测试 Excel，把历轮修过的数据边角全塞进去，保存为 samples/complex_test.xlsx。
200 行 × 33 命名列（含 1 个重复列名 dup）；另含 配置/数值表/边界/空表 多个 sheet。
...
以及（本轮新增，"内容多一点"）：多行单元格(内嵌 \\n\\t)、真实日期(→ISO 字符串)、JSON 字符串(须保字符串不被解析)、
空格填充数字串(两侧空格不应被吞)、真实大整数(数值，区别于文本 big_id)、单列混合类型(int/str/None)。

用法（backend 目录）：PYTHONIOENCODING=utf-8 python tools/make_complex_excel.py
"""
import json
import random
...
QUESTIONS = ["什么是勾股定理？", "水的沸点是多少？", "光速约为多少？", "氢的原子序数？",
             ...
             "什么是熵增定律？", "声音在空气中的速度？", "质数有哪些性质？", "什么是光合作用？",
             "万有引力公式是什么？", "汉朝建立于哪一年？", "什么是黑体辐射？", "细胞的基本结构？"]
CATEGORIES = ["数学", "物理", "化学", "历史", "生物", "天文", "地理", "计算机"]
# 乱码/多脚本样本（均为合法 unicode，避开 openpyxl 非法控制字符 \x00-\x1f）
MOJI = ["😀🎉中文ABC", "Привет мир", "مرحبا بالعالم", "日本語テキスト한국어",
        "zero​width", "combińing", "rtl‮mirror", "﻿BOM在中间",
        "ℝ𝕖𝕒𝕝𝕥𝕪𝕡𝕖", "🇨🇳🇺🇸旗帜", "㊙️㊗️圈字符", "tab\there"]
LONG = "长" * 32000  # 近 Excel 单元格上限 32767，测超长值

rows = []
for i in range(N):
    rows.append({
        "id": f"{i + 1:03d}",                                  # 前导零文本：001..200
        ...
        "multiline": f"第一行{i}\n第二行\t制表位",                # 多行单元格：内嵌换行/制表
        "ts": BASE_TS + pd.Timedelta(days=i),                  # 真实日期 → 读回应是 ISO 字符串
        "json_str": json.dumps({"k": i, "arr": [1, 2, 3]}, ensure_ascii=False),  # JSON 串须保字符串不被解析
        "space_pad": f" {i % 100} ",                           # 空格填充的数字串：两侧空格不应被吞
        "big_num": 10 ** 15 + i,                               # 真实大整数（数值，区别于文本 big_id）
        "mixed": [i, "n/a", None, f"s{i}"][i % 4],             # 单列混合类型 int/str/None
...
df_cfg = pd.DataFrame([{"category": c, "weight": round((i + 1) * 0.1, 1),
                        "enabled": "true" if i % 2 else "false",
                        "note": MOJI[i % len(MOJI)]}                          # 配置表带乱码备注列
                       for i, c in enumerate(CATEGORIES)])                     # 不同 schema 的配置表
df_num = pd.DataFrame([{"seq": i, "ratio": i / 3, "big": 10 ** 14 + i, "label": f"L{i:02d}"}
                       for i in range(12)])                                   # 真数值(int/float/大整数 保数值)
df_edge = pd.DataFrame([{"边界值": "", "说明": "全空单元格"},
                        {"边界值": "   ", "说明": "纯空白"},
                        {"边界值": "😀{{x}}", "说明": "emoji+模板字面量"}])       # 小而怪的边界 sheet（3 行）
...
with pd.ExcelWriter(OUT) as w:
    ...
    df_edge.to_excel(w, sheet_name="边界", index=False)
    df_empty.to_excel(w, sheet_name="空表", index=False)
print(f"主数据 列数={df.shape[1]}（含重复 dup）行数={df.shape[0]}；另有 配置/数值表/边界/空表 sheet")
