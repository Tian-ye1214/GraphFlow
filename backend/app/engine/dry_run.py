...
from app.agent.data_preview import (WorkflowDataPreview, _columns_from_rows, _fit_budget,
                                    _safe_rows)
...
def _budgeted(result: dict) -> str:
    """把 dry_run 结果裁到 wrap_tools 20k 预算内再序列化（给 agent 工具用）。"""
    key = "output_rows" if "output_rows" in result else "rows"
    return json.dumps(_fit_budget(result, key=key), ensure_ascii=False)


def make_dry_run_tools(session_factory, user_id: int, workflow_id: int, node_id: str,
                       current_config: dict | None = None) -> list:
    """节点助手(llm_synth/qc)试跑工具：用前端未保存的草稿 config(current_config)在真实输入小样本上试跑。"""
    async def try_node_config(call_model: bool = True) -> str:
        """试跑当前正在配置的节点：用真实输入的小样本(≤3 行)跑该节点逻辑，看渲染后的提示词与模型实际产出/质检判定。
        据真实产出迭代提示词。零副作用(不落库、不计入正式运行、不耗费正式额度统计)。
        Parameters:
            call_model: True 调真实模型看产出(消耗 token)；False 只看提示词渲染成什么样(免费,不调模型)
        """
        try:
            result = await dry_run_node(session_factory, user_id, workflow_id, node_id,
                                        override_config=current_config, call_model=call_model)
        except (DryRunNotFound, ValueError) as e:
            return json.dumps({"error": str(e)}, ensure_ascii=False)
        return _budgeted(result)
    return [try_node_config]


def make_codegen_dry_run_tools(session_factory, user_id: int, workflow_id: int, node_id: str) -> list:
    """智能处理(auto_process)代码生成试跑工具：把候选 process(rows) 代码在真实输入小样本上跑一遍看产出。"""
    async def try_process_code(code: str) -> str:
        """在真实输入的小样本(≤20 行)上试跑候选 process(rows) 代码，看转换后的产出行/列，返回前确认代码能跑通。
        零副作用(不落库)。代码必须定义 process(rows: list[dict]) -> list[dict]。
        Parameters:
            code: 候选 Python 代码，需定义 process(rows) 函数
        """
        try:
            result = await dry_run_node(
                session_factory, user_id, workflow_id, node_id,
                override_config={"operations": [{"op": "agent", "code": code}]}, limit=LOCAL_SAMPLE_CAP)
        except (DryRunNotFound, ValueError) as e:
            return json.dumps({"error": str(e)}, ensure_ascii=False)
        return _budgeted(result)
    return [try_process_code]
