...
**动机**：跑数本质是有向有环图——质检后回扫形成环。这个过程要大量调提示词、
小量调链路。目标模式让 Agent 自我迭代，直到跑数质检首轮通过率达标。

**典型用法**：用户说「把这条流水线调到首轮质检通过率 ≥90%」。Agent 进入目标循环：
跑流水线 → 采样评估产出（`gf export -o` 到会话目录 → read_file → 按目标标准评估，
必要时用 LLM 自评）→ 计算通过率 → 没达标就改提示词（`gf node set llm_synth_1 prompt=…`）
或微调链路 → 再跑 → 直到达标或到轮上限。每轮在聊天里报告本轮通过率与所做调整。
...
- 目标模式指引（移植 goal_mode 提示词并平台化）：用户给出可度量目标时进入目标循环
  （跑→采样评估→报告通过率→调提示词/链路→续轮），每轮末尾输出 CONTINUE/DONE 标记；
  评估优先用规则（长度/格式/字段齐全），语义质量用 LLM 自评抽样 ≤20 条控制成本
...
7. 目标模式：给出「通过率 ≥X%」类目标后 Agent 自动多轮迭代（脚本化模型可重现
   跑→评估→改提示词→再跑循环），轮上限与【停止】按钮均生效
