...
ImageGeneration、goal_mode、PyInstaller 打包、RedLotus 自有 config.json 模型配置层、
akshare/matplotlib 等技能附带依赖。

**保留并移植：**
三角色编排（含 Worker 并行波次与共享消息板）、文件工具、run_command（黑名单沿用）、
todo 工具、skills 工具（list/get/load/execute）、search_web（ddgs）、
...
- 砍掉原提示词中的记忆/CLI 交互/goal_mode 相关段落
...
- 状态：进行中显示停止位提示（status=running 时输入框禁用，显示「红莲正在工作…」）
...
3. SSE payload 扩展 + 流式事件 + 后台续跑/重启恢复
4. 前端 AgentDrawer（消息流/工具条目/确认按钮/模型选择）
5. 三角色全量端到端 + 审批硬拦专测 + 删除 RedLotus/ 目录 + README
